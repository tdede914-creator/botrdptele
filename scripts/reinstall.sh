...
set -eE
confhome=https://raw.githubusercontent.com/tdede914-creator/reinstall/main
confhome_cn=https://jihulab.com/tdede914-creator/reinstall/-/raw/main
# confhome_cn=https://www.ghproxy.cc/https://raw.githubusercontent.com/tdede914-creator/reinstall/main
...
is_use_local_grub() {
    # Linode legacy Ubuntu uses GRUB, but the MBR signature check can fail on
    # Linode virtual disks. When tele.sh exports LINODE_FORCE_GRUB=1, force
    # all local bootloader logic into the GRUB path and never EXTLINUX.
    if [ "${LINODE_FORCE_GRUB:-0}" = "1" ] && is_use_local_grub_extlinux; then
        return 0
    fi
...
is_use_local_extlinux() {
    # Do not let Linode fall into EXTLINUX. Ubuntu Linode images normally have
    # /boot/grub/grub.cfg, while extlinux_cfg is empty and causes:
    # dirname: missing operand.
    if [ "${LINODE_FORCE_GRUB:-0}" = "1" ]; then
        return 1
    fi
    is_use_local_grub_extlinux && ! is_mbr_using_grub
}
...
is_mbr_using_grub() {
    find_main_disk
    # Linode legacy boot sering tidak menulis signature GRUB di 440 byte pertama MBR,
    # sehingga script salah memilih jalur EXTLINUX dan gagal karena extlinux.conf kosong.
    # Kalau tele.sh sudah mendeteksi Linode, paksa pakai jalur GRUB lokal.
    if [ "${LINODE_FORCE_GRUB:-0}" = "1" ]; then
        return 0
    fi
    # Jika grub.cfg ada, gunakan jalur GRUB walaupun signature MBR tidak terbaca.
    if find /boot/grub /boot/grub2 -type f -name grub.cfg 2>/dev/null | head -1 | grep -q .; then
        return 0
    fi
...
find_main_disk() {
    ...
    if is_in_windows; then
        ...
        # 获取 xda 的 id
        install_pkg fdisk
        main_disk=$(fdisk -l /dev/$xda | grep -i 'Disk identifier' | awk '{print $NF}' | sed 's/0x//' | sed 's/[{}]//g')

        # Linode and some cloud images may not print a Disk identifier via fdisk.
        # Try other common sources before failing. The downstream installer needs
        # a stable disk id in extra_main_disk, so do not leave it empty.
        if ! grep -Eix '[0-9a-f]{8}' <<<"$main_disk" &&
            ! grep -Eix '[0-9a-f-]{36}' <<<"$main_disk"; then
            main_disk=$(blkid -o value -s PTUUID /dev/$xda 2>/dev/null | head -n1 | sed 's/0x//' | sed 's/[{}]//g')
        fi
        if ! grep -Eix '[0-9a-f]{8}' <<<"$main_disk" &&
            ! grep -Eix '[0-9a-f-]{36}' <<<"$main_disk"; then
            main_disk=$(lsblk -dn -o PTUUID /dev/$xda 2>/dev/null | head -n1 | sed 's/0x//' | sed 's/[{}]//g')
        fi
        if ! grep -Eix '[0-9a-f]{8}' <<<"$main_disk" &&
            ! grep -Eix '[0-9a-f-]{36}' <<<"$main_disk"; then
            install_pkg sfdisk
            main_disk=$(sfdisk --disk-id /dev/$xda 2>/dev/null | head -n1 | awk '{print $NF}' | sed 's/0x//' | sed 's/[{}]//g')
        fi

        # If the disk really has no identifier, create one. This fixes Linode
        # templates that return: MAIN DISK: SDA, Disk ID is invalid: <empty>.
        if ! grep -Eix '[0-9a-f]{8}' <<<"$main_disk" &&
            ! grep -Eix '[0-9a-f-]{36}' <<<"$main_disk"; then
            pttype=$(blkid -o value -s PTTYPE /dev/$xda 2>/dev/null | head -n1 | tr '[:upper:]' '[:lower:]')
            if [ "$pttype" = gpt ]; then
                new_disk_id=$(cat /proc/sys/kernel/random/uuid 2>/dev/null || uuidgen 2>/dev/null)
                if [ -n "$new_disk_id" ]; then
                    sfdisk --disk-id /dev/$xda "$new_disk_id" >/dev/null 2>&1 || true
                fi
            else
                new_disk_id=$(od -An -N4 -tx4 /dev/urandom 2>/dev/null | tr -d ' 
')
                if [ -n "$new_disk_id" ]; then
                    sfdisk --disk-id /dev/$xda "0x$new_disk_id" >/dev/null 2>&1 || true
                fi
            fi
            sync || true
            main_disk=$(sfdisk --disk-id /dev/$xda 2>/dev/null | head -n1 | awk '{print $NF}' | sed 's/0x//' | sed 's/[{}]//g')
    ...
    if ! grep -Eix '[0-9a-f]{8}' <<<"$main_disk" &&
        ...
        ! { [ "${LINODE_FORCE_MAIN_DISK_DEVICE:-0}" = "1" ] && grep -Eq '^/dev/(sd[a-z]|vd[a-z]|xvd[a-z]|nvme[0-9]+n[0-9]+)$' <<<"$main_disk"; }; then
        # Linode sometimes ships a valid single-disk image without a readable
        # Disk Identifier/PTUUID. The initrd still requires extra_main_disk,
        # so for single-disk Linode pass the Linux device path explicitly.
        # This fixes LISH error: "cmdline main_disk is empty".
        if [ "${LINODE_FORCE_MAIN_DISK_DEVICE:-0}" = "1" ] && [ "$(wc -l <<<"$xda")" -eq 1 ]; then
            main_disk="/dev/$xda"
            echo "Warning: Disk ID is empty on single-disk Linode. Using main disk device: $main_disk"
        else
...
脚本有更新，请重新下载 reinstall.sh"
    ...
    # Linode/Xen compatibility:
    # The original system may report /dev/sda, but after booting the Alpine
    # initrd on Linode the same disk is often exposed as /dev/xvda. The
    # upstream trans.sh normally treats extra_main_disk as a disk identifier,
    # so passing /dev/sda can fail with: "Could not find xda: /dev/sda".
    # Patch trans.sh so an explicit device path/name is accepted and resolved
    # to the first real Linode block device visible inside Alpine.
    if [ "${LINODE_FORCE_MAIN_DISK_DEVICE:-0}" = "1" ]; then
        python3 - "$initrd_dir/trans.sh" <<'PY' || true
from pathlib import Path
import re, sys
path = Path(sys.argv[1])
text = path.read_text()
patch_func = r'''
linode_resolve_main_disk_device() {
    case "${main_disk:-}" in
        /dev/*|sd[a-z]|vd[a-z]|xvd[a-z]|nvme[0-9]n[0-9]|xvd[a-z][0-9]*|sd[a-z][0-9]*|vd[a-z][0-9]*)
            for _linode_disk in /dev/xvda /dev/sda /dev/vda /dev/nvme0n1; do
                if [ -b "$_linode_disk" ]; then
                    xda="${_linode_disk##*/}"
                    main_disk="$xda"
                    echo "Linode disk override: using /dev/$xda"
                    return 0
                fi
            done
            ;;
...
'''
if 'linode_resolve_main_disk_device()' not in text:
    text = patch_func + '\n' + text
# Patch find_xda when present. This is the most reliable place because
# upstream trans.sh may otherwise treat main_disk as an ID and reject /dev/sda.
patterns = [r'(^find_xda\s*\(\)\s*\{)', r'(^find_xda\s*\{)']
patched = False
for pat in patterns:
    m = re.search(pat, text, flags=re.M)
    if m and 'linode_resolve_main_disk_device && return 0' not in text[m.end():m.end()+300]:
        text = text[:m.end()] + '\n    linode_resolve_main_disk_device && return 0\n' + text[m.end():]
        patched = True
        break
# Also patch the main entry as a fallback for trans.sh variants without find_xda.
if re.search(r'^: main', text, flags=re.M) and 'linode_resolve_main_disk_device || true' not in text:
    text = re.sub(r'(^: main.*$)', r'\1\nlinode_resolve_main_disk_device || true', text, count=1, flags=re.M)
...
if is_need_grub_extlinux; then
    ...
    if is_in_windows; then
        ...
        if is_efi; then
            ...
            if is_mbr_using_grub; then
                ...
                # Linode fallback: some Ubuntu images have GRUB but update-grub
                # path detection returns empty. Pick the real grub.cfg directly.
                if [ -z "$grub_cfg" ] && [ "${LINODE_FORCE_GRUB:-0}" = "1" ]; then
    ...
    if is_use_local_grub; then
        ...
        [ -n "$grub_cfg" ] || error_and_exit "grub.cfg not found for local GRUB boot"
